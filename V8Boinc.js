...
var tmp = require; //TODO

var di = require('./lib/DI.js'),
  defaults = tmp('./lib/defaults.js'),
  xml = tmp('./lib/XmlHelper.js'),
  md5 = tmp('MD5'),
  InputFile = tmp('./lib/InputFile.js'),
  V8BoincBatch = tmp('./lib/V8BoincBatch.js'),
  request = tmp('request');
...
module.exports = function (options, sniffer, logger) {
  ...
  this.submitJobs = function (jobs, batch_name, done) {
    function serializeJob(job) {
      ...
      if (!(job.in_json instanceof InputFile)) {
        return "job.in_json should be instance of InputFile";
      }
      if (!(job.main_js instanceof InputFile)) {
