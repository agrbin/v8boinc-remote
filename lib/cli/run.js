...
module.exports.main = function (app, done) {
  ...
  function submit(callback) {
    ...
    function stageFile(path, done) {
      var desc, mode = 'path', dir = null, url = null;
      if (app.options.publicdir) {
        desc = app.options.publicdir;
        mode = "semilocal";
        dir = desc.split(':')[0];
        url = desc.split(':').slice(1).join(':');
      }
      fh.stageFile(mode, dir, url, path, done);
    }

    function addJob(path, callback) {
      async.series([
        stageFile.bind(null, app.bundle.bundledMainJs),
        stageFile.bind(null, path),
