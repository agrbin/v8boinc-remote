...
function getV8Boinc(app, authenticated, done) {
  var v8b = new app.V8Boinc(
    {
      authenticated : authenticated,
      project: app.globalConfig.get('project', defaults.defaultProject, true),
...
function requireV8Boinc(app, authenticated, done) {
  getV8Boinc(app, authenticated, function (err, v8boinc) {
    if (err) {
      app.cli.error(err);
