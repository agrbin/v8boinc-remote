...
module.exports = function Bundle(app, onFinish) {
      ...
      function (err) {
        if (err) {
          if (options.hold) {
            ...
            app.cli.info("hint: just do rm -rf .v8boinc/bundles/*");
            return app.cli.fatal(err);
          }
        }
        ...
        async.series([
          fs.writeFile.bind(null, bundlePath, bundlePreamble),
          fs.appendFile.bind(null, bundlePath, data, onWriteFile),
        ], onWriteFile);
      }
    );
  }
  ...
  function startBundling() {
    ...
    // this we use for emscripten compiled stuff to work.
    browserify.exclude('ws');
