...
module.exports.main = function (app, done) {
  ...
  function extrapolate(err, duration, done) {
    var matches = err.stderrTxt.match(/__v8boinc__bfd__\|([.0-9]+)\|/),
      p;
    if (duration < 1000) {
      app.cli.error("test went too fast. estimate may be wrong");
    }
    if (!matches) {
      done(
  ...
  function runOne(file, done) {
    ...
    jsappExec.run(
      ...
      function (err) {
        ...
        extrapolate(err, new Date().getTime() - timer - 2000, done);
      }
    );
  }
  ...
  function isItOk(fpops) {
    ...
    if (fpops < 2e9) {
      app.cli.error("estimated fpops too small. " + best);
    }
    if (fpops > 1e11) {
      app.cli.error("estimated fpops too large. " + best);
    }
  ...
  common.requireAppConfig(app, function () {
    app.options.preJs = 'var boinc_fraction_done = '
      + boinc_fraction_done.toString() + ';\n';
