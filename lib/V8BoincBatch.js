...
function V8BoincBatch(gateway, logger) {
  ...
  function submitHelper(jobs, done) {
    gateway.submitJobs(jobs, Random.getName('batch_'), function (err, batchId) {
      ...
      if (!batchId || !batchId.batch_id) {
        return done("batchId not retrieved. error when submitting");
      }
  ...
  this.submit = function (submitProgress, onSubmitCallback) {
    ...
    function finish() {
      ...
      return onSubmit(errors.length ? errors : null, submitted);
    }
